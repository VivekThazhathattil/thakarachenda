...
function App() {
  ...
  const themeToggler = (themeString) => {
    ...
    if (allThemes.includes(themeString)) setTheme(themeString);
    ...
    <ThemeProvider theme={() => applyTheme()}>
      ...
      <StyledApp className="App">
        <button
          ...
          style={{ background: "#7B5537", borderColor: "white" }}
          onClick={() => themeToggler("paper")}
        />
        <button
          className="color-icon"
          style={{ background: "#e6e6e6", borderColor: "white" }}
          onClick={() => themeToggler("plain")}
        />
        <button
          className="color-icon"
          style={{ background: "#cc0022", borderColor: "white" }}
          onClick={() => themeToggler("cherry")}
        />
        <button
          className="color-icon"
          style={{ background: "#2F5233", borderColor: "white" }}
          onClick={() => themeToggler("green")}
        />
        <button
          className="color-icon"
          style={{ background: "#E97ACB", borderColor: "white" }}
          onClick={() => themeToggler("dark")}
        />
        <button
          className="color-icon"
          style={{ background: "#808080", borderColor: "white" }}
          onClick={() => themeToggler("amoled")}
        />
        <LoadTitle />
        <LoadIndex />
        <LoadData />
...
function LoadSongData() {
  return (
    <>
      ...
      <p>
        ഒരുപാട് കേൾക്കുമ്പോൾ ഞാൻ എന്റെ playlist-ൽ ഉള്ള പാട്ടുകൾ ഒക്കെ delete
        ചെയ്യും. എന്നിട്ട് ഇതുവരെ കേൾക്കാത്തതൊക്കെ തിരഞ്ഞു പിടിച്ച്‌ കേൾക്കും.
        ഒരിക്കൽ delete ചെയ്തുകഴിഞ്ഞ പാട്ട് പിന്നെ ആകസ്മികമായി തിരിച്ചു
        വരാറുണ്ട്. ഒരു പഴയ സുഹൃത്തിനെ പോലെ കണ്ട്, ലോഗ്യമൊക്കെ ചോദിച്ച് ഞാൻ അവന്
        പറയാനുള്ളത് വീണ്ടും കേൾക്കും. Unlike ചെയ്ത് delete ചെയ്യുന്നതിന് മുൻപ്
        ഞാൻ അവരുടെ പേരൊക്കെ ഇവിടെ ചേർക്കും. കാലം കുറെ കഴിയുമ്പോൾ അവരെയൊക്കെ
        വീണ്ടും വിളിച്ചു സൽക്കരിക്കാലോ.
      </p>
      <div className="grid-container">
        {songs.map((song, idx) => {
          const divStyle = {
            backgroundImage:
              `linear-gradient( rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 1) ), url("/images/album_covers/` +
              song.coverimg +
              `")`,
            color: "white",
          };
...
function AssembleBlogDetails({ id, year, month, day, time, title }) {
  ...
  return (
    <>
      <h3 id={tag} className="h3-title">
        {title}
      </h3>
      <p id={tag} className="no-capital colored-p">
        {day}/{month}/{year} - ({time})
      </p>
      <hr />
      <LazyLoadComponent>
...
function ImagesElement({ imgKey }) {
  ...
  if (imgKey === "horsey") return <Horsey />;
  else
    return (
